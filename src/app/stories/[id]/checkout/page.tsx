...
const TIER_LABELS: Record<string, string> = {
  digital: 'Digital Keepsake',
  print: 'Printed Storybook',
  gift: 'Gift Edition',
  undecided: 'Printed Storybook',
};
...
export default function CheckoutPage() {
  ...
  const price = product?.price || '29.00';
  const tierLabel = TIER_LABELS[product?.productType || 'undecided'] || 'Printed Storybook';
            ...
            <Card border="green">
              <div className="p-8 sm:p-12 text-center">
                ...
                <p
                  ...
                  style={{ color: '#7B6E90' }}
                >
                  Your book is now unlocked. We&apos;re generating illustrations
                  for every page — this usually takes a few minutes.
                </p>
                <div
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          >
            <Card>
              <CardHeader
                ...
                title="Order Summary"
                subtitle="What you're unlocking"
              />
              ...
              <div className="px-6 pb-5 space-y-2.5">
                <Feature icon={<Palette className="w-3.5 h-3.5" />}>
                  HD illustrations for every page
                </Feature>
                <Feature icon={<Wand2 className="w-3.5 h-3.5" />}>
                  Style-locked to your chosen aesthetic
                </Feature>
                <Feature icon={<BookOpen className="w-3.5 h-3.5" />}>
                  Studio editing + print-ready PDF
                </Feature>
              </div>

              <PriceRow label={tierLabel} value={`£${price}`} />
              <TotalRow label="Total" value={`£${price}`} />
            </Card>
          </motion.div>
          ...
          >
            <Card>
                    ...
                    <PayPalButtons
                      ...
                      createOrder={async () => {
                        const res = await fetch('/api/paypal/order', {
                          ...
                          body: JSON.stringify({
                            storyId,
                            product: `FlipWhizz ${tierLabel}: ${story.title}`,
                            price,
                            currency: 'GBP',
                          }),
                        });
                      ...
                      onApprove={async (data) => {
                        ...
                        try {
                          ...
                          if (result.success) {
                            await fetch(
                              `/api/stories/${storyId}/complete-step`,
                              {
                                method: 'POST',
                                headers: {
                                  'Content-Type': 'application/json',
              ...
              >
                <TrustBadge icon={<Lock className="w-3 h-3" />}>
                  Encrypted
                </TrustBadge>
...
function PriceRow({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      ...
      <span className="text-sm font-bold" style={{ color: '#2D2235' }}>
        {value}
      </span>
    </div>
  );
