...
export default function CoverDesignChat({
    ...
    const interval = setInterval(async () => {
      ...
      if (data.story?.coverSpreadUrl) {
        ...
        const autoMessage: ChatMsg = {
          role: "assistant",
          content: "Your cover is ready! Take a look at the preview — what do you think? Feel free to ask for any changes.",
        };
  ...
  useEffect(() => {
    ...
    if (hasCovers) {
      ...
      setMessages([{
        role: "assistant",
        content: "Here's your current cover! If you'd like to make any changes — different characters, new background, updated text — just let me know and I'll regenerate it for you.",
      }]);
      return;
    }
  ...
  async function finalizeCoverPlan() {
    ...
    try {
      ...
      const lockMessage: ChatMsg = {
        role: "assistant",
        content: "Perfect — your cover design is locked. Generating artwork now, this may take 30–60 seconds…",
      };
  ...
  async function handleRegenerateWithFeedback(feedback: string) {
    ...
    try {
      ...
      const regenMsg: ChatMsg = {
        role: "assistant",
        content: "Got it! Regenerating your cover with those changes — about 30–60 seconds…",
      };
        ...
        <main className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
            <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold mb-3" style={{ background: "rgba(199,125,255,0.1)", color: "#9B59D0" }}>
              <Palette className="w-3.5 h-3.5" /> Cover Designer
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold mb-2" style={{ color: "#2D2235", letterSpacing: "-0.03em" }}>Design Your Cover</h2>
            <p className="text-sm sm:text-base max-w-lg mx-auto leading-relaxed" style={{ color: "#7B6E90" }}>Chat about your vision — colours, mood, layout — and I'll bring it to life.</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
            ...
            <div className="lg:col-span-3">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="overflow-hidden rounded-[22px]" style={{ background: "white", border: "1px solid rgba(180,150,210,0.12)", boxShadow: "0 2px 12px rgba(100,60,140,0.06)" }}>
                {/* Chat bar */}
                ...
                {/* Messages */}
                <div className="h-[480px] sm:h-[520px] overflow-y-auto p-5 space-y-4 scrollbar-hide" style={{ background: "rgba(249,245,255,0.25)" }}>
                  ...
                  {messages.length === 0 && !isLoading && (
                    <div className="flex flex-col items-center justify-center h-full text-center">
                      <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-5" style={{ background: "rgba(199,125,255,0.1)" }}>
                        <Sparkles className="w-8 h-8" style={{ color: "#C77DFF" }} />
                      </div>
                      <h3 className="text-xl font-extrabold mb-2" style={{ color: "#2D2235" }}>Let's Design Your Cover</h3>
                      <p className="text-sm max-w-sm leading-relaxed" style={{ color: "#7B6E90" }}>Tell me about your vision and I'll create a cover that brings your story to life.</p>
                    </div>
                  )}
                  {messages.map((m, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className={`flex items-start gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                      {m.role === "assistant" ? <AvatarAssistant /> : <AvatarUser />}
                      <Bubble side={m.role === "user" ? "right" : "left"}><p className="whitespace-pre-wrap">{m.content}</p></Bubble>
                    </motion.div>
                  ))}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <Bubble side="left">
                        <div className="flex items-center gap-1.5">
                          {[0, 150, 300].map((delay) => (<span key={delay} className="w-2 h-2 rounded-full animate-bounce" style={{ background: "#C4A8E0", animationDelay: `${delay}ms` }} />))}
                        </div>
                      </Bubble>
                    </div>
                  )}
                ...
                <div className="p-4" style={{ borderTop: "1px solid rgba(180,150,210,0.08)" }}>
                  {!hasCovers ? (
                    <div className="space-y-3">
                      ...
                      <button onClick={finalizeCoverPlan} disabled={isFinalising || messages.length < 2 || isGeneratingCovers} className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-xl text-sm font-bold text-white transition-all disabled:opacity-40 active:scale-[0.98]" style={{ background: "linear-gradient(135deg, #B05CE6, #D45DA0)", boxShadow: "0 4px 16px rgba(176,92,230,0.25)", border: "none", fontFamily: "inherit" }}>
                        {isFinalising || isGeneratingCovers ? (<><Loader2 className="w-4 h-4 animate-spin" />{isGeneratingCovers ? "Generating…" : "Creating Design…"}</>) : (<><Wand2 className="w-4 h-4" />Finalise &amp; Generate Cover</>)}
                      </button>
                    </div>
                  ) : (
                    <div className="space-y-3">
            ...
            <div className="lg:col-span-2">
              <div className="sticky top-24 space-y-4">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="overflow-hidden rounded-[22px]" style={{ background: "white", border: "1px solid rgba(180,150,210,0.12)", boxShadow: "0 2px 12px rgba(100,60,140,0.06)" }}>
                  <div className="flex items-center gap-3 px-5 py-3.5" style={{ borderBottom: "1px solid rgba(180,150,210,0.08)", background: "rgba(249,245,255,0.5)" }}>
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "rgba(199,125,255,0.1)" }}>
                  ...
                  <div className="p-5">
                    {hasCovers && !isGeneratingCovers ? (
                      <div className="space-y-4">
                        <div className="relative overflow-hidden rounded-2xl" style={{ boxShadow: "0 8px 30px rgba(100,60,140,0.12), 0 2px 8px rgba(100,60,140,0.06)", border: "1px solid rgba(180,150,210,0.1)" }}>
                          <img src={localStory.coverSpreadUrl!} alt="Cover preview" className="w-full" />
                        </div>
                        <button onClick={handleApprove} className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-xl text-sm font-bold text-white transition-all active:scale-[0.98]" style={{ background: "linear-gradient(135deg, #43B89C, #2FA482)", boxShadow: "0 4px 16px rgba(67,184,156,0.25)", border: "none", fontFamily: "inherit" }}>
                          <Check className="w-4 h-4" /> Approve &amp; Continue
                        </button>
                        <p className="text-center text-xs" style={{ color: "#A897BD" }}>or request changes in the chat</p>
                      </div>
                    ) : isGeneratingCovers ? (
                      <div className="flex flex-col items-center justify-center h-64 rounded-2xl" style={{ background: "linear-gradient(135deg, rgba(249,245,255,0.8), rgba(255,240,248,0.6))", border: "1px solid rgba(180,150,210,0.1)" }}>
                        <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 3, ease: "linear" }} className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4" style={{ background: "linear-gradient(135deg, #B05CE6, #D45DA0)", boxShadow: "0 4px 16px rgba(176,92,230,0.25)" }}>
                          <Wand2 className="w-6 h-6 text-white" />
                        </motion.div>
                        <p className="text-sm font-bold" style={{ color: "#2D2235" }}>Creating your cover…</p>
                        <p className="text-xs mt-1" style={{ color: "#A897BD" }}>This takes about 30–60 seconds</p>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center justify-center h-64 rounded-2xl" style={{ border: "2px dashed rgba(180,150,210,0.2)", background: "rgba(249,245,255,0.3)" }}>
                        <ImagePlus className="w-10 h-10 mb-3" style={{ color: "#D4C6E6" }} />
                        <p className="text-sm font-medium" style={{ color: "#A897BD" }}>Your cover will appear here</p>
                        <p className="text-xs mt-1" style={{ color: "#C4B5D4" }}>once you finalise the design</p>
                      </div>
                    )}
                  </div>
                </motion.div>
...
function WorldReferencePanel({ characters, locations, loading }: { characters: WorldCharacter[]; locations: WorldLocation[]; loading: boolean }) {
  ...
  if (loading) {
    return (
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-[22px] p-5" style={{ background: "white", border: "1px solid rgba(180,150,210,0.12)", boxShadow: "0 2px 12px rgba(100,60,140,0.06)" }}>
  ...
  if (characters.length === 0 && locations.length === 0) {
    return (
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-[22px] p-5 text-center" style={{ background: "white", border: "1px solid rgba(180,150,210,0.12)", boxShadow: "0 2px 12px rgba(100,60,140,0.06)" }}>
        <User className="w-6 h-6 mx-auto mb-2" style={{ color: "#C4A8E0" }} />
        <p className="text-xs font-medium" style={{ color: "#A897BD" }}>Character references will appear here as we discuss your cover</p>
      </motion.div>
    );
  }

  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="overflow-hidden rounded-[22px]" style={{ background: "white", border: "1px solid rgba(180,150,210,0.12)", boxShadow: "0 2px 12px rgba(100,60,140,0.06)" }}>
      <button onClick={() => setExpanded((e) => !e)} className="w-full flex items-center justify-between gap-3 px-5 py-3.5 text-left" style={{ borderBottom: expanded ? "1px solid rgba(180,150,210,0.08)" : "none", background: "rgba(249,245,255,0.5)" }}>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "rgba(199,125,255,0.1)" }}>
            <User className="w-4 h-4" style={{ color: "#B05CE6" }} />
          </div>
          <div>
            <p className="text-sm font-bold" style={{ color: "#2D2235" }}>Story World</p>
...
function CharacterCard({ character }: { character: WorldCharacter }) {
  ...
  return (
    <div className="flex gap-3 p-3 rounded-xl" style={{ background: "rgba(249,245,255,0.5)", border: "1px solid rgba(180,150,210,0.08)" }}>
      {character.imageUrl ? (
        <img src={character.imageUrl} alt={character.name} className="w-12 h-12 rounded-xl object-cover flex-shrink-0" style={{ border: "1px solid rgba(180,150,210,0.15)" }} />
      ) : (
        <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(199,125,255,0.1)" }}>
          <User className="w-5 h-5" style={{ color: "#C4A8E0" }} />
        </div>
      )}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="text-sm font-bold truncate" style={{ color: "#2D2235" }}>{character.name}</p>
          {character.role && (<span className="text-[10px] px-1.5 py-0.5 rounded-full font-semibold flex-shrink-0" style={{ background: "rgba(199,125,255,0.1)", color: "#9B59D0" }}>{character.role}</span>)}
        </div>
        {character.appearance && (<p className="text-[11px] mt-0.5 line-clamp-2 leading-relaxed" style={{ color: "#7B6E90" }}>{character.appearance}</p>)}
        {defaultOutfit && (
          <div className="flex items-center gap-1 mt-1.5">
            <Shirt className="w-3 h-3 flex-shrink-0" style={{ color: "#C4A8E0" }} />
            <p className="text-[10px] truncate" style={{ color: "#A897BD" }}>{defaultOutfit.outfitDescription}</p>
          </div>
        )}
        {character.outfits.length > 1 && (<p className="text-[10px] mt-0.5" style={{ color: "#C4B5D4" }}>+{character.outfits.length - 1} other outfit{character.outfits.length - 1 !== 1 && "s"}</p>)}
      </div>
    </div>
  );
}
...
function LocationCard({ location }: { location: WorldLocation }) {
  return (
    <div className="flex gap-3 p-3 rounded-xl" style={{ background: "rgba(249,245,255,0.5)", border: "1px solid rgba(180,150,210,0.08)" }}>
      {location.imageUrl ? (
        <img src={location.imageUrl} alt={location.name} className="w-12 h-12 rounded-xl object-cover flex-shrink-0" style={{ border: "1px solid rgba(180,150,210,0.15)" }} />
      ) : (
        <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(67,184,156,0.08)" }}>
          <MapPin className="w-5 h-5" style={{ color: "#43B89C" }} />
        </div>
      )}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="text-sm font-bold truncate" style={{ color: "#2D2235" }}>{location.name}</p>
          {location.significance && (<span className="text-[10px] px-1.5 py-0.5 rounded-full font-semibold flex-shrink-0" style={{ background: "rgba(67,184,156,0.1)", color: "#2FA482" }}>{location.significance}</span>)}
        </div>
        {location.description && (<p className="text-[11px] mt-0.5 line-clamp-2 leading-relaxed" style={{ color: "#7B6E90" }}>{location.description}</p>)}
      </div>
    </div>
  );
}
...
function AvatarAssistant() {
  return (
    <div className="w-8 h-8 flex-shrink-0 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #B05CE6, #D45DA0)", boxShadow: "0 2px 8px rgba(176,92,230,0.2)" }}>
...
function AvatarUser() {
  return (
    <div className="w-8 h-8 flex-shrink-0 rounded-xl flex items-center justify-center" style={{ background: "rgba(199,125,255,0.1)" }}>
...
function Bubble({ side, children }: { side: "left" | "right"; children: React.ReactNode }) {
  return (
    <div className={`max-w-[78%] px-4 py-3 text-sm leading-relaxed ${side === "right" ? "rounded-2xl rounded-tr-md text-white" : "rounded-2xl rounded-tl-md"}`}
      style={side === "right" ? { background: "linear-gradient(135deg, #B05CE6, #D45DA0)", boxShadow: "0 3px 12px rgba(176,92,230,0.2)" } : { background: "white", color: "#2D2235", border: "1px solid rgba(180,150,210,0.12)", boxShadow: "0 1px 4px rgba(100,60,140,0.04)" }}>
      {children}
    </div>
  );
}
...
function SendButton({ onClick, disabled, loading }: { onClick: () => void; disabled: boolean; loading: boolean }) {
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{ background: "linear-gradient(135deg, #B05CE6, #D45DA0)", boxShadow: "0 3px 12px rgba(176,92,230,0.2)", border: "none", fontFamily: "inherit" }}>
