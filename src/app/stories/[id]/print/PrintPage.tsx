        ...
        {story.coverSpreadUrl && (
          ...
          >
            <div
              ...
              style={{
                boxShadow: "0 12px 48px rgba(100,60,140,0.15), 0 2px 8px rgba(100,60,140,0.08)",
        ...
        >
          ...
          >
            {hasOrder
              ...
              : isDigitalOnly
                ? "Your print-at-home PDF is ready. Download it below and follow the simple instructions to make your book."
        ...
        >
          ...
          {isDigitalOnly && (
            <button
              ...
              className="w-full flex items-center justify-center gap-2.5 py-4 rounded-xl text-base font-bold text-white transition-all active:scale-[0.98] disabled:opacity-50"
              style={{
          ...
          {/* ── PHYSICAL: Print at Home (secondary) ── */}
          {isPhysical && (
            <button
  ...
  return (
    ...
    >
      <div
        ...
        style={{ borderBottom: "1px solid rgba(180,150,210,0.08)", background: "rgba(249,245,255,0.5)" }}
      >
      ...
      <div className="px-5 py-4 space-y-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-semibold" style={{ color: "#2D2235" }}>Order placed</span>
        </div>
        <div className="flex items-center gap-3">
          <StatusDot active={["processing", "shipped", "delivered"].includes(order.gelatoStatus ?? "")} done={["shipped", "delivered"].includes(order.gelatoStatus ?? "")} />
          <span className="text-xs font-semibold" style={{ color: ["processing", "shipped", "delivered"].includes(order.gelatoStatus ?? "") ? "#2D2235" : "#C4B5D4" }}>Printing</span>
        </div>
        <div className="flex items-center gap-3">
          <StatusDot active={["shipped", "delivered"].includes(order.gelatoStatus ?? "")} done={order.gelatoStatus === "delivered"} />
          <span className="text-xs font-semibold" style={{ color: ["shipped", "delivered"].includes(order.gelatoStatus ?? "") ? "#2D2235" : "#C4B5D4" }}>Shipped</span>
        </div>
        {order.gelatoOrderId && (
          <p className="text-[11px] pt-1" style={{ color: "#A897BD" }}>Order ref: {order.gelatoOrderId}</p>
        )}
      </div>
    </div>
  );
...
function StatusDot({ active, done }: { active: boolean; done: boolean }) {
  return (
    <div
      ...
      style={{
        background: done ? "rgba(67,184,156,0.15)" : active ? "rgba(176,92,230,0.12)" : "rgba(180,150,210,0.08)",
        border: done ? "2px solid #43B89C" : active ? "2px solid #B05CE6" : "2px solid rgba(180,150,210,0.15)",
      }}
    >
      ...
      <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ duration: 0.38, ease: [0.16, 1, 0.3, 1] }} className="w-full max-h-[92vh] overflow-y-auto" style={{ background: "white", borderRadius: "24px 24px 0 0", fontFamily: FONT }}>
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            ...
            <InputField label="Postcode" value={address.postCode} onChange={(v) => update("postCode", v)} error={errors.postCode} disabled={isSubmitting} />
          </div>
