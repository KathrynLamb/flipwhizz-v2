...
interface Props {
  ...
  userEmail: string; // ← added — pass from session server component
}
...
function getTimelineIndex(status: string): number {
  const idx = TIMELINE_STEPS.findIndex((s) => s.matchStatuses.includes(status));
  return idx >= 0 ? idx : 0;
}
...
// Styled to match the ActionCard grid but opens the ShareToInstagram sheet
function InstagramShareCard({
  ...
  return (
    <>
      {/* The card — matches ActionCard visual exactly */}
      <button
        onClick={() => setOpen(true)}
        className="text-left w-full relative flex flex-col items-start gap-3 p-6 rounded-2xl border-[1.5px] border-gray-200 bg-white transition-all duration-200 cursor-pointer hover:-translate-y-[3px] hover:shadow-lg hover:border-pink-300"
      >
        <span
          className="absolute top-3 right-3 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wide"
        ...
        <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-pink-50 text-[#D94590]">
        ...
        <div>
          ...
          <div className="text-[13px] leading-relaxed text-gray-500">
            Share {story.childName}'s story and get 40% off your next book.
          </div>
        </div>
      </button>

      {/* The actual sheet — rendered at root level */}
      <ShareToInstagram
...
// ─── Main Component ───
export default function CompletedBookView({ story, order, userEmail }: Props) {
    ...
    <div className="min-h-screen font-sans">
      ...
      <div className="max-w-[960px] mx-auto px-6 pt-10 pb-20">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-4 animate-in fade-in slide-in-from-bottom-4 duration-600 delay-200">
          ...
          {/* 4. Share to Instagram ← replaces the old link-copy Share card */}
          <InstagramShareCard story={story} userEmail={userEmail} />
          ...
          <ActionCard
            ...
            variant="accent"
            href={story.worldId ? `/stories/new?worldId=${story.worldId}` : "/stories/new"}
          />
        </div>
          ...
          <div className="mt-10 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-600 delay-400">
            <div className="p-7 bg-white rounded-2xl border border-gray-200">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-[15px] font-semibold text-gray-900">Print Order</h3>
              </div>
                    ...
                    <div key={step.key} className="flex items-center flex-1">
                      <div className="flex flex-col items-center gap-2 min-w-[80px]">
                        ...
                        <span className={`text-xs ${
                          isComplete ? "text-green-500 font-semibold"
                      ...
                      {i < TIMELINE_STEPS.length - 1 && (
                        <div className={`flex-1 h-0.5 -mx-2 mb-7 transition-colors duration-300 ${
...
function OrderDetailsPanel({ order }: { order: OrderData }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      ...
      >
        <h3 className="text-[15px] font-semibold text-gray-900">Order Details</h3>
        <div className="flex items-center gap-2 text-gray-400">
          <span className="text-xs">{order.createdAt ? formatDate(order.createdAt) : ""}</span>
        ...
        <div className="px-7 pb-6 pt-0 border-t border-gray-100">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-5">
            ...
            <div className="space-y-3">
              ...
              <DetailRow label="Order ref" value={order.gelatoOrderId ? order.gelatoOrderId.slice(0, 8).toUpperCase() : order.id.slice(0, 8).toUpperCase()} />
            ...
            {addr && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-[13px] font-semibold text-gray-700 mb-2">
                  <MapPin size={14} className="text-gray-400" /> Shipping To
                </div>
                <div className="text-[13px] text-gray-600 leading-relaxed space-y-0.5">
                  {(addr.firstName || addr.lastName) && (
                    <p className="font-medium text-gray-800">{[addr.firstName, addr.lastName].filter(Boolean).join(" ")}</p>
                  )}
                  {addr.addressLine1 && <p>{addr.addressLine1}</p>}
                  {addr.addressLine2 && <p>{addr.addressLine2}</p>}
                  {(addr.city || addr.postCode) && <p>{[addr.city, addr.postCode].filter(Boolean).join(", ")}</p>}
                  {addr.countryIsoCode && <p>{formatCountry(addr.countryIsoCode)}</p>}
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-[13px] font-semibold text-gray-700 mb-2">
                <Calendar size={14} className="text-gray-400" /> Delivery
              </div>
              {formatDeliveryRange(order.minDeliveryDate, order.maxDeliveryDate) ? (
                <DetailRow label="Estimated" value={formatDeliveryRange(order.minDeliveryDate, order.maxDeliveryDate)!} />
              ) : (
                <p className="text-[13px] text-gray-400 italic">Delivery estimate available once printing begins</p>
              )}
              {order.trackingCode && <DetailRow label="Tracking" value={order.trackingCode} />}
              {addr?.email && (
                <div className="pt-3 mt-3 border-t border-gray-100">
                  <div className="flex items-center gap-2 text-[13px] font-semibold text-gray-700 mb-2">
                    <Mail size={14} className="text-gray-400" /> Contact
                  </div>
                  <p className="text-[13px] text-gray-600">{addr.email}</p>
                  {addr.phone && <p className="text-[13px] text-gray-600 mt-0.5">{addr.phone}</p>}
                </div>
              )}
            </div>
