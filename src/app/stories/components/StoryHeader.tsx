...
export default function UnifiedStoryHeader({
  ...
  const rawCompletedSteps = Array.isArray(completedSteps)
    ...
    console.log("HAs pages", hasPages, storyConfirmed)
  ...
  // Find the active step for mobile display
  const activeStep =
    STEPS.find((s) => mounted && s.match(pathname, storyId)) || STEPS[0];
  const activeStepIndex = stepIndex(activeStep.key);
    ...
    <>
      ...
      >
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          {/* ─── DESKTOP ─── */}
          <div className="hidden md:block">
            {/* Top row: back + title + action + user menu */}
            <div className="flex items-center justify-between py-3">
              {/* Left: Back + Title */}
              <div className="flex items-center gap-4 min-w-0">
                <button
              ...
              {/* Right: Progress/Generate + User Menu */}
              <div className="flex items-center gap-3 flex-shrink-0">
                {showProgress &&
                  progressTotal !== undefined &&
                  progressTotal > 0 && (
          ...
          <div className="md:hidden">
            ...
            <div className="flex items-center justify-between h-14">
              ...
              <button
                onClick={() => setMobileNavOpen(!mobileNavOpen)}
                className="flex items-center gap-2 px-3 py-1.5 rounded-xl max-w-[50%]"
                style={{
                  background: "rgba(180,150,210,0.06)",
                  border: "1px solid rgba(180,150,210,0.1)",
                  cursor: "pointer",
