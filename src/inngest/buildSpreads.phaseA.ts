...
const MODEL = "claude-3-5-sonnet-20240620"; // Updated to latest stable Sonnet
...
// Helper to match "Sophia" to "Princess Sophia"
function findCharId(nameToFind: string, charMap: Record<string, string>): string | undefined {
  // 1. Exact match
  if (charMap[nameToFind]) return charMap[nameToFind];

  // 2. Case insensitive
  const lowerToFind = nameToFind.toLowerCase();
  const exactKey = Object.keys(charMap).find(k => k.toLowerCase() === lowerToFind);
  if (exactKey) return charMap[exactKey];

  // 3. Partial match (e.g. "Sophia" matches "Princess Sophia")
  const partialKey = Object.keys(charMap).find(k => k.toLowerCase().includes(lowerToFind) || lowerToFind.includes(k.toLowerCase()));
  if (partialKey) return charMap[partialKey];
    ...
    try {
      ...
      const ai = await step.run("claude-analyze", async () => {
        ...
        const SCHEMA_DEF = JSON.stringify(
          {
            spreads: [
              {
                spreadIndex: 0,
                sceneSummary: "Visual description of the scene.",
                pages: [
                  {
                    page: "left", 
                    location: "Name of location from ALLOWED LOCATIONS",
                    characters: [
                      {
                        name: "Name of character from ALLOWED CHARACTERS",
                        prominence: "primary", 
                        justification: "Why they are here",
                      },
                    ],
        ...
        const res = await anthropic.messages.create({
          ...
          system: `
You are a children's picture book story editor.
Analyze the story text and output a JSON plan for the illustrations.

CRITICAL INSTRUCTIONS:
1. Output ONLY valid JSON matching the schema below.
2. Use the EXACT schema provided.
3. **PRONOUN RESOLUTION**: If the text says "She", "He", or "The Princess", you MUST infer which specific character from the ALLOWED CHARACTERS list this refers to. Do not omit characters just because they are referred to by pronoun.
4. **INFERRED PRESENCE**: If a character is in the scene but not speaking (e.g., "Sophia and the Bear walked..."), ensure BOTH are listed on the page.
...
${storyText}
        ...
        // Fallback / Recovery logic similar to before...
        if (!parsed || !Array.isArray(parsed.spreads)) {
             // (Reuse your existing recovery regex logic here if desired, omitted for brevity but highly recommended to keep)
             throw new Error("Claude returned unrecoverable JSON");
        }
      ...
      await step.run("persist", async () => {
        ...
        await db.transaction(async (tx) => {
          ...
          for (const spread of ai) {
            const pair = pairs.find((p) => p.spreadIndex === spread.spreadIndex);
            if (!pair) continue;
            ...
            for (const pageInfo of spread.pages) {
              const page = pageInfo.page === "left" ? pair.left : pair.right;
              if (!page) continue;

              // Fuzzy match location
              const locationId = findCharId(pageInfo.location, locByName);
              if (locationId) {
                await tx.insert(storyPageLocations).values({
                  id: uuid(),
                  pageId: page.id,
              ...
              for (const c of pageInfo.characters) {
                // Fuzzy match character (The critical fix)
                const characterId = findCharId(c.name, charByName);
                
                if (!characterId) {
                    console.warn(`⚠️ Could not map AI character "${c.name}" to DB characters`);
                    continue;
                }
      ...
      await step.run("dispatch-phase-b", async () => {
        if (!ai.length) return;
        await inngest.send({
          name: "story/generate.spreads", // Updated event name to match your new Phase B
          data: { storyId },
        });
      });
